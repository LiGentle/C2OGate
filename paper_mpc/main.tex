...
proof construction, envelope acquisition, and verification.
...
faster in the reported front-end comparison.  The implemented increment is the
cost-violating cell disjunction, three-valued aggregation, proof producer and
small consumer, and explicit cost ledger; it is not a new general PEP language
or interpolation theorem.
...
PEPit \PepitVersion{} and ran both front ends serially through Clarabel.  The
version is the installed public distribution and its metadata points to the
official \url{https://pepit.readthedocs.io/en/0.5.1/} documentation.  At
...
in a separate MATLAB/YALMIP environment.
...
Nor does the present evidence show that a real repeated workload frequently
lands in the flagship displacement bands.  The five-envelope family tests
mathematical and producer robustness across declared envelopes, not their
empirical frequency.  A trust-region or multifidelity model manager may make
better practical accept/reject choices and already supplies convergence
safeguards; C2OGate adds only a separately checkable pointwise cost claim when
its stronger transcript and funding conditions hold.  No experiment here
demonstrates that it changes a production trust-region decision.
