...
relative to an unchanged baseline.  Conditioning on one first-order
transcript, it couples both continuations on the same unknown function,
enumerates their joint stopping-time cells, and represents each cell over
smooth strongly convex functions by a dimension-free Gram SDP.  Acceptance
requires independently verified exclusion of every cost-violating cell; a
verified attainable bad cell rejects, and every unresolved numerical status
returns \emph{uncertified}.  We prove the finite reduction, characterize the
fixed-dimension rank qualification, and show that independent marginal call
bounds can be arbitrarily conservative even for one-dimensional quadratics.
The artifact includes a non-shift, nonquadratic $H=3$ acceptance whose proof
ledger contains a recovered rational dual for a generic Gram-SDP cell: a
standard-library verifier reconstructs the SDP, checks a negative dual upper
bound, and proves ten positive leading principal minors.  A generic $H=20$
run is reported only as a solver-witnessed diagnostic with proof outcome
\emph{uncertified}.  A 2,000-family quadratic audit is explicitly a
constructed finite proxy, and the full-revelation real-SPX quadratic is a
constant-and-cost-pipeline stress test rather than a short-transcript PEP
result.  Measured certificate cost is charged separately and is self-financing
only for sufficiently expensive calls or offline reuse.  All guarantees remain
conditional on certified transcripts, oracle contracts, dimensions, and cost
units.
...
\begin{enumerate}
  ...
  nonquadratic $H=3$ transcript, the artifact recovers an exact rational dual
  for the generic cell $(3,3)$; its standard-library verifier reconstructs the
  full Gram SDP, dual stationarity, objective, and positive-definite slack.
  Longer-horizon generic dual recovery is not claimed.
  ...
  $\cF_{\mu,L}$ quadratics.
  \item The artifact includes structured and generic $H=20$ enumerations,
  exact nonlinear and rational-quadratic acceptance ledgers, and independent
  verifiers.  Its 2,000 finite quadratic families are a constructed proxy
  (hence optimistic for an infinite consistency class), and its
  full-revelation SPX quadratic tests constant and cost acquisition rather
  than short-transcript PEP certification.
\end{enumerate}
...
$T_\alpha=I-\alpha\nabla f$ in \eqref{eq:gd} is $q$-Lipschitz.  One way to
see this is to restrict $\nabla f$ to the segment
...
$I-\alpha\bar H$ has norm at most $q$.  The integral representation along the
segment gives this identity for every differentiable $L$-smooth strongly
convex function.  Because
...
Conversely, if $\tau_{rs}^*>0$, not all feasible margins can be nonpositive;
hence the feasible set contains a point with some $\tau>0$.  The interpolation
...
$d<p$, it rejects only when its Gram rank is at most $d$ or an independently
reconstructed $d$-dimensional witness is supplied; otherwise it is a warning,
not a realized counterexample.  Acceptance remains safe without the rank
constraint because the SDP is an outer problem.  In operational form,
...
detects the applicable regularity case.
\end{proposition}
...
\begin{proposition}[A verified generic cell exclusion]
\label{prop:generic-dual}
Let $\mu=9/10$, $L=\alpha=\gamma=1$, $R=6/5$,
$24/25\leq\norm{y-x}\leq97/100$,
$\norm{y-x+\nabla f(x)}\leq1/100$, and $\eps=7/50$.
For the non-shift $H=3$ joint PEP, cell $(3,3)$ satisfies
\begin{equation}
 \tau_{33}^*\leq
 -\frac{208087769186767}{10765669781400000}<0.       \label{eq:generic-dual-bound}
\end{equation}
This exclusion is certified by an ordinary rational SDP dual and is valid for
the complete class $\cF_{9/10,1}$ consistent with the stated transcript.
\end{proposition}

\begin{proof}
The cell uses four gradients on each trajectory, one minimizer atom, and the
proposal atom, hence a Gram matrix of order ten.  The artifact reconstructs
all 86 rational affine inequalities: pairwise interpolation, proposal and
distance bounds, two terminal inequalities, six common-margin survival
inequalities, and the derived trace bound
\[
 4R^2+4(R+97/100)^2+R^2+(97/100)^2<27.
\]
Indeed, $L=1$ and nonexpansiveness of both gradient-descent trajectories give
four gradient-atom bounds by $R$, four by $R+97/100$, while the minimizer and
proposal atoms are bounded by $R$ and $97/100$, respectively.  Summing their
squared norms gives the displayed strict bound.  Hence the trace inequality is
a redundant consequence of the original cell constraints and does not change
the feasible set or the cell semantics.
The stored nonnegative rational multipliers and one free equality multiplier
satisfy exact stationarity for the nine function values and $\tau$.  Their
dual objective is the right-hand side of \eqref{eq:generic-dual-bound}.  The
resulting rational slack is positive definite by Sylvester's criterion: all
ten leading principal minors are strictly positive.  Weak duality therefore
proves \eqref{eq:generic-dual-bound}.  The standard-library verifier recomputes
each coefficient, identity, minor, and objective from the declared transcript;
the floating search status is not used in this proof.
\end{proof}
...
We timed the $H=3$ conic search and rational reconstruction once and ran the
independent verifier in five separate processes to estimate its median charge.
...
\begin{table}[H]
\centering
\caption{Machine-specific break-even for the recovered generic PEP dual.  The
...
Thus the recovered certificate pays for itself in one accepted decision when
an exact call costs at least ten seconds, in two reused decisions at one
...
Nonquadratic $H=3$ & \NonlinearPepCells{} cells,
\NonlinearPepBadCells{} bad & Exact acceptance & Cell $(3,3)$ has a recovered
rational generic PEP dual; the other bad cells use exact contraction bounds.\\
...
smooth strongly convex interpolation PEP at $H=3$.  Its sole positive-margin
cell is $(1,0)$.  The acceptance ledger does not merely cite the contraction
argument for every exclusion: cell $(3,3)$ is sent through the generic Gram-SDP
dual-recovery path.  That cell has a $10\times10$ Gram block, nine function
values, 86 inequalities, and one equality.  A bounded-denominator rational
reconstruction gives the exact dual upper bound
$\GenericDualUpper{}<0$.  The independent verifier reconstructs every
interpolation, transcript, terminal, and strict-margin coefficient; checks
nonnegative multipliers and exact stationarity; and proves the rational slack
positive definite through \GenericDualMinors{} positive leading principal
minors.  A redundant trace bound of 27 is independently derived from $R$, the
proposal envelope, and gradient-descent contraction and gives strict PSD
slack without changing the cell semantics.

The other nine cost-violating cells are excluded by the rational contraction
bounds above.  Thus the end-to-end ledger combines one generic recovered PEP
dual with analytic exclusions and never promotes the original
\texttt{optimal\_inaccurate} search status to proof.  The realized gradients
are $0.9761594$ at $x$, $0.0238401$ after the baseline step, and $0.0338393$
at $y$, agreeing with the class-wide result.
...
recovers part of the dependence benefit but still loses
...
horizon three.  Obtaining comparably tight low-cost constants from short
...
cells.  The nonlinear $H=3$ example now recovers and independently verifies a
full generic Gram-SDP dual for cell $(3,3)$ and combines it with analytic
exclusions for the remaining nine bad cells.  This closes the missing
small-scale end-to-end path, not generic dual recovery at $H=20$.  Longer
...
$H=3$ acceptance contains one recovered and independently verified generic
PEP dual.  The finite-family study is a constructed proxy, and the real-SPX
...
witness tampering.  The generic-dual test reconstructs the full rational Gram
SDP and exercises rehashed changes to its transcript, a dual multiplier, a
Gram coefficient, and a principal minor.  The nonlinear-acceptance test
