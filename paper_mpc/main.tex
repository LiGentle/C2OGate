...
and incomplete evidence returns \emph{uncertified}.  The consumer rebuilds
every SDP coefficient from the declared parameters in exact rational
arithmetic before checking the supplied dual.  We prove the finite
...
marginals.  Five natural-$H=10$ envelopes yield 330 exact bad-cell exclusions:
198 are independently recovered across three distinct source
envelopes and 132 follow by checked homogeneous transport.  The flagship
profile has Gram order at most 24 and 1,584 exact positive $LDL^\top$ pivots;
an independent SymPy rational-arithmetic check confirms all 1,584 exact
$LDL^\top$ pivots.  Its exact marginal rectangle also accepts, at boundary value
zero.  A separate nonzero-radius example over the full infinite class
$\cF_{1/2,1}$ is accepted only by the joint PEP.  Rational certificate
construction remains heuristic success-or-fail; only independent verification
and the resulting decisions are guaranteed, and the generic $H=20$
enumeration remains uncertified.  The deployment ledger separately charges
proof construction, envelope acquisition, and verification.  In a synthetic
rolling nonlinear logistic workload, the frozen $H=10$ proof accepts
\RollingLogisticAccepted{} of \RollingLogisticEpisodes{} decisions using only
short transcripts; post-decision full-model evaluation finds zero accepted
violations and a prepaid-proof cost ratio of \RollingLogisticWarmRatio{} after
charging every \RollingLogisticMinibatchPercent{} minibatch proposal.
...
cell models used below.  Our computational object conditions on an observed
...
their joint attainable stopping-time set.  The contribution is a software and
computational architecture that evaluates this object with explicit failure
semantics, cost accounting, and independently replayable proofs:
...
\begin{enumerate}
  ...
  verifier independently rebuilds every interpolation, transcript, stopping,
  margin, anchor, and trace coefficient from the declared parameters before
  checking dual stationarity, objective, slack, and 1,584 exact positive
  $LDL^\top$ pivots.  Two additional source envelopes are recovered
  independently and two scaled envelopes are transported exactly, giving
  \HtenFamilyExactCells{} exclusions across \HtenFamilyProfiles{} profiles.
  Rational recovery is an explicitly heuristic, success-or-fail producer;
  certificate verification is the guaranteed part.  A separate ten-cell suite
  proves joint-only acceptance over a full infinite $\cF_{1/2,1}$ transcript
  class with nonzero proposal radius.
...
The artifact additionally includes an exact marginal audit showing that the
flagship rectangle accepts at value zero, a SymPy exact-arithmetic cross-check,
five-repeat Clarabel/SCS scaling runs,
a same-model PEPit comparison, structured and generic $H=20$ diagnostics,
an exact-proposal rolling logistic simulation, and exact nonlinear and
rational-quadratic ledgers.  Its 2,000 finite quadratic
...
\paragraph{Performance estimation and conditioning.}
PEP supplies the interpolation machinery used here
\citep{DroriTeboulle2014,TaylorPEP2017,TaylorInterpolation2017}.
PESTO automated the modeling layer \citep{TaylorPESTO2017}, and PEPit is a
general Python implementation \citep{GoujaudPEPit2024}.  Operator-splitting PEP
already couples several oracle relations
\citep{RyuTaylorBergelingGiselsson2020}; BnB-PEP obtains certifiably global
method-design solutions by branch and bound \citep{DasGuptaVanParysRyu2024}.
Data-driven PEP conditions distributional guarantees on sampled trajectories
\citep{ParkRanjanStellato2025}.  These works preclude a claim that coupling,
conditioning, or certified PEP computation is new by itself.  Our narrower
object is a deterministic one-transcript branch comparison with a
discontinuous stopping rule.  PEPit 0.5.1 encodes the same signed cells and is
faster in the reported same-model numerical comparison.  It would be a natural
floating-point producer front end.  We do not make it the acceptance boundary
because the consumer must reconstruct stable, named rational coefficient rows
from parameters, reject missing or unknown rows, and replay the dual identity
without importing PEPit, CVXPY, or a conic solver.  The present artifact
therefore retains a small domain-specific exact builder; a future PEPit adapter
could emit the same proof schema without changing the consumer.  The
implemented increment is the cost-violating cell disjunction, three-valued
aggregation, proof producer and small consumer, and explicit cost ledger; it is
not a new general PEP language or interpolation theorem.

\paragraph{Rational recovery and verified SDP.}
Rigorous SDP postprocessing predates this work.  Interval methods bound
optimal values from numerical conic solutions \citep{JanssonChaykinKeil2007};
SPECTRA treats small rational LMIs in exact arithmetic
\citep{HenrionNaldiSafeyElDin2016}.  Davis and Papp recover exact dual
certificates for weighted sum-of-squares cones \citep{DavisPapp2022}, while
Roux, Voronin, and Sankaranarayanan validate polynomial-invariant SDPs using
rational reconstruction and exact positive-semidefiniteness checks
\citep{RouxVoroninSankaranarayanan2018}.  C2OGate claims neither a general
recovery theorem nor a replacement for these systems.  Its heuristic producer
is successful-or-fail; the guaranteed component is the domain-specific
consumer, which reconstructs each PEP dual identity over $\mathbb Q$ and
requires every cost-violating cell to be excluded.  Section~\ref{sec:numerics}
reports an independent SymPy exact-arithmetic cross-check and states why VSDP
and SPECTRA are not runnable dependencies of the clean Python artifact.

\paragraph{Checkable and interpretable proofs.}
The phrase ``proof carrying'' follows the producer--consumer separation of
proof-carrying code \citep{Necula1997}; we use the analogy, not its machine-code
safety model.  Recent work sparsifies PEP duals into simpler proof structures
\citep{ThomsenEtAl2026}, and certificate-to-proof compilation can verify
rational SDP evidence inside Lean in another domain
\citep{JeongEtAl2026}.  Our certificates remain dense numerical discoveries
checked by a small exact program; they are independently replayable but not
formalized or optimized for human interpretability.

\paragraph{Contribution boundary.}
The mathematical reductions below assemble standard smooth strongly convex
interpolation, a finite stopping-time disjunction, and weak duality.  The paper
is therefore evaluated as a software and computational architecture: which
cells must be solved, what evidence permits acceptance, how incomplete recovery
fails closed, how the intended conic model is reconstructed independently of
the producer, and how every paid component enters the branch ledger.
Table~\ref{tab:method-position} summarizes that boundary.  Linear-operator
interpolation \citep{Bousselmi2024} could extend the architecture but is not
used in the verified implementation.
...
The proof payload does not supply authoritative primal coefficient matrices.
For each declared parameter tuple and cell label, the standard-library
consumer independently rebuilds the vector atoms, the two affine
...
boundary; the trusted kernel includes the smaller, independently implemented
...
measured proof-kernel total is therefore \HtenCertificateTotalSeconds{} seconds;
...
\begin{table}[H]
\centering
\caption{Machine-specific break-even for the flagship natural-$H=10$ suite.
The proof-only column charges search, recovery, and replay.  The deployment
column additionally charges a 12-exact-call envelope acquisition, so the
headline accounting includes the constants used by the PDE illustration.}
\label{tab:h10-certificate-cost}
\begin{tabular}{rrr}
\toprule
Exact-oracle seconds $t_f$ & Proof-only $K$ & With envelope $K$\\
\midrule
$1$    & \HtenCertificateBreakEvenOneSecond{} & \PdeBreakEvenOneSecond{}\\
$10$   & \HtenCertificateBreakEvenTenSeconds{} & \PdeBreakEvenTenSeconds{}\\
$60$   & \HtenCertificateBreakEvenSixtySeconds{} & \PdeBreakEvenSixtySeconds{}\\
$600$  & \HtenCertificateBreakEvenSixHundredSeconds{} & \PdeBreakEvenSixHundredSeconds{}\\
$3600$ & \HtenCertificateBreakEvenHour{} & \PdeBreakEvenHour{}\\
\bottomrule
\end{tabular}
\end{table}

This flagship suite is not a one-use certificate unless its proof-kernel wall time
...
excluded by \cref{prop:zero-credit}.  The right column is the all-in headline
for the stated deployment envelope; no constant-acquisition charge is kept in
a separate accounting table.
...
The executable artifact separates numerical producers from ten
...
\midrule
Nonzero-radius $H=10$ family & \HtenFamilyProfiles{} envelopes,
\HtenFamilyExactCells{} bad cells & Exact class-wide acceptance &
\HtenFamilyIndependentCells{} exclusions recovered independently across three
source envelopes; \HtenFamilyTransportedCells{} follow by exact homogeneous
transport.  The balanced source has \HtenDualPivots{} positive exact
$LDL^\top$ pivots.\\
...
Rolling nonlinear logistic workload & \RollingLogisticEpisodes{} episodes,
\RollingLogisticSamples{} rows, dimension \RollingLogisticDimension{} &
\RollingLogisticAccepted{} short-transcript acceptances; zero post-decision
violations & Reuses the exact $H=10$ suite after checking its envelope.  The
\RollingLogisticMinibatchPercent{} proposals are charged; the workload is
synthetic and its anchor distribution is chosen to cross the certified band.\\
...
separately rebuilt all SDP rows from the parameter tuple---including 506
ordered interpolation inequalities and, at the largest cell, 28 additional
inequalities plus the anchor equality---then reconstructed 66 stationarity
systems, 66 rational slacks, and \HtenDualPivots{} positive $LDL^\top$ pivots.
Weak duality therefore gives
...
\paragraph{Five-envelope exact family.}
To test whether recovery is confined to one normalization, we repeat the full
66-cell construction on two further source envelopes.  The
``candidate-heavy'' profile uses $R=0.9$ and
$0.89\leq\norm{y-x}\leq0.91$; the ``tight-contract'' profile keeps the
balanced geometry but reduces the residual radius to $\delta=0.08$.  Together
with the balanced source, all \HtenFamilyIndependentCells{} cell duals are
recovered independently.  Their aggregate ordered searches make
\HtenFamilyRecoveryAttempts{} attempts with \HtenFamilyRecoveryFailures{}
failed attempts before exact success; the largest verified upper bounds for
the two new sources are \HtenCandidateHeavyUpper{} and
\HtenTightContractUpper{}.  Two more envelopes scale every distance,
residual, and tolerance by $4/5$ or $6/5$, and the trace bound by the squared
scale.  The verifier checks that dual stationarity and slack are invariant and
that each objective scales by the square factor.  Hence the family contains
\HtenFamilyProfiles{} exact envelopes and \HtenFamilyExactCells{} verified
profile-cell exclusions, of which \HtenFamilyTransportedCells{} use algebraic
transport rather than a second recovery claim.

\paragraph{Flagship marginal baseline.}
The balanced flagship does \emph{not} require joint information for
acceptance.  A separate three-point PEP maximizes
$\norm{\nabla f(y)}^2$ over exactly the same transcript and has exact rational
dual upper bound \HtenMarginalGradientUpper{}$<(2/3)^2$; hence $U_y=0$.
Conversely,
$\norm{\nabla f(x)}\geq79/100-1/10=69/100>2/3$, so $L_x\geq1$.
The quadratic $f(t)=(t-4/5)^2/2$, $x=0$, $y=4/5$ attains
$(N_f(x),N_f(y))=(1,0)$, proving $L_x=1$ and $U_y=0$.  Thus the exact rectangle
value is $-1$ and its cost gate equals
$-1+\max\{2/5,1\}=\HtenMarginalRectangleValue{}$.  The flagship demonstrates
moderate-horizon proof production; strict joint-over-marginal value is shown
only by the full-class experiment in \cref{sec:full-class-joint-only}.
...
We also feed SCS duals, rather than Clarabel duals, into the same heuristic
rational producer on \ScsRecoveryCells{} stratified flagship cells.  Exact
recovery succeeds on \ScsRecoverySuccesses{} and fails on
\ScsRecoveryFailures{}; every success is replayed by the standard-library
consumer, while any failure would leave the corresponding cell uncertified.
This diagnostic measures producer portability and does not promote an SCS
status to proof.
...
For a same-model comparison, we encoded exactly the same signed-margin cells
in PEPit \PepitVersion{} and ran both implementations serially through the same
CVXPY--Clarabel stack.  The optional artifact extra pins
\texttt{PEPit==\PepitVersion{}}; the frozen environment records installer
\texttt{uv}, the distribution metadata's tagged GitHub download URL, the PyPI
release URL, and the official
\url{https://pepit.readthedocs.io/en/0.5.1/} documentation.  No
\texttt{direct\_url.json} is present, so we do not infer which package-index
mirror served the wheel.

At $H=10$, PEPit takes \PepitHtenSerialWall{} seconds versus
\CtwoHtenSerialWall{} seconds, an end-to-end factor
\PepitHtenEndToEndRatio{}, and both return the same maximum margin to six
significant digits.  This is not a ``pure front-end'' ratio.  Of the
\PepitHtenExtraWall{}-second median end-to-end gap, the corresponding median
component differences are \PepitHtenExtraBuild{} seconds for native Python
model assembly, \PepitHtenExtraCanonicalization{} seconds for wrapper/CVXPY
canonicalization, and \PepitHtenExtraLoop{} seconds for loop and measurement
overhead.  The native-minus-PEPit Clarabel-kernel difference is
\PepitHtenExtraSolverKernel{} seconds (negative means native is faster).
PEPit is therefore the better floating modeling
front end in this test.  We retain the native proof path only because C2OGate's
distinct layer requires named exact coefficient reconstruction, three-valued
cell aggregation, rational recovery, and a solver-independent consumer.  PESTO
is cited as the 2017 MATLAB predecessor; we do not present it as a 2022 chordal
solver or duplicate the Python benchmark in a separate MATLAB/YALMIP
environment.

\paragraph{Independent exact-arithmetic backend.}
As a full-load consumer cross-check, SymPy's rational matrix backend computes
an exact $LDL^\top$ decomposition of all \SympyCrosscheckSlacks{} flagship
dual slacks.  All \SympyCrosscheckPivots{} exact diagonal pivots are positive,
independently confirming positive definiteness.  VSDP was not run
because the clean artifact has neither MATLAB/Octave nor its interval stack
(the public implementation is at
\url{https://github.com/vsdp/vsdp-2020}); SPECTRA requires Maple and its
official project page, \url{https://homepages.laas.fr/henrion/software/spectra/},
states that the package is no longer maintained (accessed August 2026).
These are toolchain limitations, not performance comparisons.
The public payload records the executable audit and the SymPy version, while
the standard-library consumer remains the acceptance boundary.
...
\subsection{Rolling nonlinear calibration: acceptance from a short transcript}
\label{sec:rolling-logistic}

We next test the flagship proof as a reusable gate in a nonlinear repeated
workload, rather than reveal a quadratic model to the decision rule.  The
simulation has \RollingLogisticEpisodes{} rolling proximal logistic
calibrations, each with \RollingLogisticSamples{} normalized feature rows and
\RollingLogisticDimension{} parameters.  For episode $e$,
\begin{equation}
 f_e(w)=\frac{0.8}{n}\sum_{i=1}^n
 \left[\log(1+\exp(a_i^\top w))-b_i a_i^\top w\right]
 +\frac{0.8}{2}\norm{w-u_e}^2 .                 \label{eq:rolling-logistic}
\end{equation}
The binary labels and normalized Gaussian features are frozen by seed; the
regularization center $u_e$ changes direction and magnitude across episodes,
modeling a rolling prior or proximal-center update.  Since
$\norm{a_i}=1$ and the logistic second derivative lies in $[0,1/4]$,
\[
 0.8I\preceq\nabla^2 f_e(w)\preceq I.
\]
Thus every episode belongs to $\cF_{0.8,1}\subset\cF_{0.1,1}$ without a
full Hessian computation.

At $x=0$, the exact full-data gradient is already in the transcript.  The
external producer scans a fresh \RollingLogisticMinibatchPercent{} minibatch,
forms its gradient $\widetilde g_e$, and proposes $y_e=-\widetilde g_e$.  Before
branching, the gate checks only
\[
 0.79\leq\norm{y_e}\leq0.81,\qquad
 \norm{y_e+\nabla f_e(0)}\leq0.1,\qquad
 \frac{\norm{\nabla f_e(0)}}{0.8}\leq1.
\]
The last inequality is the strong-monotonicity distance certificate
$\norm{x-x_*}\leq\norm{\nabla f_e(x)}/0.8$.  These checks place the transcript
inside the balanced $H=10$ envelope, so its 66 independently replayed bad-cell
exclusions apply unchanged.  Candidate gradients, realized stopping times,
the optimizer, and future full-data gradients are withheld until after this
decision; they are used only for the outcome audit.

The gate accepts \RollingLogisticAccepted{} of \RollingLogisticEpisodes{}
episodes (\RollingLogisticAcceptRate{}).  In the post-decision full-model
evaluation, every accepted episode has stopping pair $(1,0)$, so the
\RollingLogisticAccepted{} acceptances save \RollingLogisticSavedCalls{} exact
calls and produce \RollingLogisticViolations{} safety violations.  The
unchanged baseline uses \RollingLogisticBaselineCalls{} remaining exact calls;
the gated paths use \RollingLogisticGatedCalls{}.  Charging all minibatch
queries adds \RollingLogisticCheapUnits{} exact-call units, giving prepaid-proof
total \RollingLogisticWarmRatio{} of baseline and net saving
\RollingLogisticWarmSaved{} units.

The one-time proof is not silently removed.  Using the measured
$H=10$ search-plus-verification charge, a 10-second exact oracle gives an
all-in ratio \RollingLogisticColdRatioTen{} on these 256 episodes and requires
\RollingLogisticBreakEvenTen{} episodes at the observed acceptance rate.  At
60 seconds the corresponding values are \RollingLogisticColdRatioSixty{} and
\RollingLogisticBreakEvenSixty{}; at 600 seconds they are
\RollingLogisticColdRatioSixHundred{} and
\RollingLogisticBreakEvenSixHundred{}.  Therefore this frozen batch is
self-financing in the 60- and 600-second scenarios, but not in the 10-second
scenario.  It does not finance a possibly rejected first decision from zero
credit.

This study closes a narrower evidence gap than a production deployment would.
It is a reproducible synthetic generalized-linear workload, not observed
operations data; its center-shock distribution is deliberately chosen to
cross the certified displacement band.  It demonstrates that a nonlinear
repeated workload can trigger the exact gate without revealing the candidate
continuation, while leaving empirical frequency and constant acquisition in a
real system open.
...
The separate accounting audit makes the safety--mean tradeoff explicit.  Its
certified gate has mean cost ratio \AccountingGateMeanRatio{}, whereas querying
the proposal unconditionally has the lower mean \AccountingAlwaysMeanRatio{}
but exceeds the baseline cost on \AccountingAlwaysWorseRate{} of cases.  The
paper's deterministic gate optimizes neither of those averages: it pays for a
pointwise non-inferiority guarantee.  A risk-neutral user of this constructed
distribution would prefer always-query; a user requiring zero audited
overruns may prefer the conservative gate.  This is a decision criterion, not
evidence that certification improves expected cost.
...
applications remains the principal practical bottleneck.  The rolling
logistic study in \cref{sec:rolling-logistic} obtains its constants from
normalized features and explicit proximal curvature; that is a genuine
short-transcript route, but not evidence that such structure is generic.
...
The rolling logistic simulation lands in the flagship band in
\RollingLogisticAcceptRate{} of its frozen episodes and quantifies rejected
proposal cost, but it is not a real repeated workload and its shock range is
selected to cross that band.  The five-envelope family tests mathematical and
producer robustness across declared envelopes, not universal empirical
frequency.  A trust-region or multifidelity model manager may make better
practical accept/reject choices and already supplies convergence safeguards;
C2OGate adds only a separately checkable pointwise cost claim when its stronger
transcript and funding conditions hold.  No experiment here demonstrates that
it changes a production trust-region decision.
...
required for acceptance.  Its consumer also rebuilds the complete finite SDP
from declared parameters, so neither producer-side coefficient assembly nor a
stored solver matrix is trusted.  Its three-valued output is deliberate: a witnessed
...
rank qualification for fixed dimension.  The natural-$H=10$ study now covers
\HtenFamilyProfiles{} envelopes and \HtenFamilyExactCells{} verified
profile-cell exclusions: \HtenFamilyIndependentCells{} are independently
recovered across three source envelopes and \HtenFamilyTransportedCells{}
follow by checked homogeneous transport.  For the balanced source, the
standard-library consumer checks \HtenDualPivots{} exact positive
$LDL^\top$ pivots, and an independent SymPy backend confirms the same number
of positive exact pivots.  Its exact marginal rectangle also accepts,
at boundary value zero, so this flagship study supports moderate-horizon proof
production rather than a claim that dependence is always essential.  A
separate exact ten-cell suite over the full infinite class $\cF_{1/2,1}$
demonstrates strict joint-over-marginal acceptance.
Repeated Clarabel/SCS runs, an SCS-to-rational recovery diagnostic, and a
same-model PEPit comparison expose numerical cost rather than hiding it, while
the contract-misspecification audit shows that optimistic declarations can
create false acceptances relative to the full consistency class.
The rolling proximal-logistic workload additionally exercises the frozen
$H=10$ proof before full revelation: \RollingLogisticAccepted{} of
\RollingLogisticEpisodes{} short transcripts accept, the held-out continuation
audit finds zero violations, and the charged prepaid-proof ratio is
\RollingLogisticWarmRatio{}.  Its cold ledger is favorable for the reported
60-second exact-oracle scenario but not for the 10-second scenario.
...
therefore does not establish a real production workload, universal
acceleration, cheap constant
...
ten independent exact replays, and metric-bound article reconstruction,
respectively.
